...
from local_gradient_math import local_gradient_alloc, local_gradient, lstsqr_lines, z_brt, z_ast, \
    get_position_brightfield, get_position_astigmatism, get_position_doublehelix
...
class ParticleDetector:
    ...
    def get_pos_astigmatism(
        ...
        for i, page in enumerate(PIL.ImageSequence.Iterator(img)):
            ...
            x_i, y_i, z_i = get_position_astigmatism(im, thrtype, thr, R, positiveAngle,
    ...
    def get_pos_doublehelix(
        ...
        for i, page in enumerate(PIL.ImageSequence.Iterator(img)):
            ...
            x_i, y_i, z_i = get_position_doublehelix(im, thrtype, thr, R, mid_rng,
                                                     (gMatxfft, gMatyfft, sMatfft), roi, z_pos)
            end = time.time()
            x = np.append(x, x_i)
            y = np.append(y, y_i)
        ...
        if draw:
            ...
            fig.update_layout(height=600, width=1400, title_text="Double-helix xyz")
            fig.show()
